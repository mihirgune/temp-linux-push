...
if [ -z "$REPO_URL" ]; then
  ...
  echo "Please provide the repository URL as an environment variable."
  echo "Usage: docker run -e REPO_URL=<your_repo_url> nondex-runner"
  exit 1
fi
...
NONDEX_LOG_FILE="$NONDEX_LOG_DIR/nondex-$(date +%Y%m%d-%H%M%S).log"
NONDEX_FAILURE_FILE="$NONDEX_LOG_DIR/nondex-failures-$(date +%Y%m%d-%H%M%S).txt"
